...
- ✅ 自动处理时区差异和历史日期查询
- ✅ 智能URL构建，支持最新和历史论文爬取
- ✅ 直接写入数据库（Supabase/Postgres），不再依赖本地 Markdown 文件
...
2. **搜索文章列表** - 爬取指定日期的论文并写入数据库
3. **开始分析** - 使用AI对论文进行深度分析（只对未分析论文补齐）
4. **查看结果** - 直接从数据库读取分析结果，支持按总分排序
...
### 数据存储说明（已移除文件依赖）
- 论文数据与分析结果全部存储在数据库中：`app.papers`、`app.categories`、`app.paper_categories`、`app.analysis_results`
- 不再生成或读取 `log/*.md` 文件；历史文件可作为归档，无需参与运行时读写

### 智能缓存（数据库版）
系统会自动检测已有的分析结果：
- 加载现有分析结果
- 增量分析（从短范围扩展到长范围）
- 避免重复分析（按 `paper_id + prompt_id` 唯一约束）
