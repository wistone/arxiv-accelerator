-- Seed basic categories and default prompt

-- categories
insert into app.categories (category_name)
values ('cs.CV'), ('cs.AI'), ('cs.LG')
on conflict (category_name) do nothing;

-- default prompt (system_default)
-- NOTE: Replace :system_prompt_content via your migration script or SQL editor
insert into app.prompts (prompt_name, prompt_content)
values ('system_default', '你是一位 "多模态大模型论文甄别官"，目标是在给定论文 Title + Abstract 时，输出可解释、可调节的相关度评价。
...
on conflict (prompt_name) do update set prompt_content = excluded.prompt_content;
